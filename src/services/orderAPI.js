...
export const orderDH1 = (lastName, firstName, email, address, phone, note, products, soTienGiamGia, giamGia, soTienCanThanhToan, thanhTien, tongSoLuong, idKhachHang) => {
...
export const orderDH = (data) => {
    const URL_BACKEND = `/api/order/dat-hang`;
    
    // Gửi trực tiếp object 'data' này lên Backend
    return axios.post(URL_BACKEND, data);
}
...
export const getMotDonHang = (query) => {
    const URL_BACKEND = `/api/order/${query}`    
    return axios.get(URL_BACKEND)
}
